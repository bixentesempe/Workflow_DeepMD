#!/usr/bin/env python3
"""
setup_sp_calculations.py
------------------------
Prepare des dossiers de calcul VASP single-point (SP) a partir de POSCARs
(typiquement issus d'une selection QBC), et genere les scripts SLURM
necessaires pour les lancer tous en job array.

Ce que fait le script, etape par etape :
  1. Trouve tous les fichiers POSCAR dans --qbc-dir (recherche recursive,
  ...
  2. Pour chaque POSCAR, cree un dossier de calcul contenant :
     ...
     (voir --copy-inputs) : ~9 Mo economises par calcul.
...
----------------------------------
  ...
  (b) MaxSubmitJobs / limite de jobs PENDING par utilisateur (~10 000).
      ...
      decoupee en plusieurs sbatch. 26 000 POSCARs a 1 calcul/tache =
      26 000 jobs en attente = sbatch rejete.
      Contournee avec --calcs-per-task N : les calculs sont groupes par N
      et executes en sequence dans la tache. 26 000 calculs a 10/tache =
      2 600 taches.
...
--------------------------
POTCAR et vdw_kernel.bindat pesent ~9 Mo a eux deux. Copies dans 5000
dossiers, ce sont 45 Go pour un contenu strictement identique. Par
defaut le script en fige UNE copie dans <output-dir>/inputs/ et place
un lien symbolique RELATIF dans chaque dossier de calcul. VASP suit les
liens en lecture : le calcul est rigoureusement identique.
...
Usage
-----
    python setup_sp_calculations.py -i ./HW2O-QBC1/poscars -o ./SP_HW2O_AL1
    python setup_sp_calculations.py -i ./qbc_poscars -o ./SP_calculations \\
                                    --potcar ./POTCAR --calcs-per-task 10
    python setup_sp_calculations.py -i ./qbc_poscars --dry-run
"""
...
QBC_POSCAR_DIR = Path("./")                  # ou chercher les POSCAR (recursif)
OUTPUT_DIR     = Path("./SP_calculations")   # ou creer les dossiers de calcul
INCAR_SP       = Path("./INCAR")
...
CALCS_PER_TASK = 1       # 1 = ancien comportement de setup_sp_calculations_array.py
...
def _setup_logging(level: int = logging.INFO) -> logging.Logger:
    logger = logging.getLogger("setup_sp")
...
def _is_prepared_calc_dir(d: Path) -> bool:
    ...
    Un POSCAR issu d'une selection QBC est seul dans son dossier.
...
def _find_poscars(
    ...
    FIX : avec les defauts (--qbc-dir ./ et --output-dir ./SP_calculations),
    le rglob du deuxieme run trouvait les POSCAR que le premier run avait
    copies dans SP_calculations/ : chaque relance doublait silencieusement
    le nombre de calculs.

    Exclure output_dir seul ne suffit pas : lancer -o SP_AL1 puis -o SP_AL2
    depuis le meme -i fait avaler SP_AL1 par le second run. D'ou le critere
    structurel _is_prepared_calc_dir.
    """
    ...
    for p in sorted(qbc_dir.rglob("POSCAR")):
        p_res = p.resolve()
    ...
    if not poscar_files:
        logger.error("Aucun POSCAR trouve dans %s", qbc_dir)
        if n_prepared or n_under_output:
            logger.error("(%d ont ete ignores comme deja prepares — "
                         "verifie --qbc-dir)", n_prepared + n_under_output)
        sys.exit(1)
...
def _calc_dir_name(poscar_path: Path, qbc_dir: Path, unique_names: bool) -> str:
    """
    Nom du dossier de calcul derive du POSCAR source.

    Par defaut : le nom du dossier parent (comportement historique).
    Avec --unique-names : le chemin relatif complet aplati, ce qui rend
    les collisions impossibles au prix de noms plus longs.
    """
    ...
    try:
        rel = parent.resolve().relative_to(qbc_dir.resolve())
    except ValueError:
        return parent.name
...
def _freeze_shared_inputs(
    ...
    Sans ce figeage, les 5000 liens pointeraient vers le POTCAR passe en
...
def _prepare_calc_dirs(
    ...
    FIX : detection des collisions de noms. Deux POSCAR issus de runs QBC
    differents peuvent avoir le meme dossier parent (ex: QBC1/poscars/
    POSCAR_rank00000_.../ et QBC2/poscars/POSCAR_rank00000_.../). Avant,
    le second ecrasait le POSCAR du premier ET dirlist.txt recevait deux
    lignes identiques — donc deux taches SLURM lancaient VASP dans le
    MEME dossier en parallele (WAVECAR corrompu, OUTCAR entrelace).
    ...
    for poscar_path in poscar_files:
        calc_dir = (output_dir / _calc_dir_name(poscar_path, qbc_dir, unique_names)).resolve()
        ...
        if not dry_run:
            ...
            shutil.copy(incar_sp,    calc_dir / "INCAR")
...
def _write_slurm_template(
    ...
    parallele). Avec calcs_per_task=1 le comportement est identique a
    l'ancien setup_sp_calculations_array.py.
...
#SBATCH --job-name=vasp_sp
#SBATCH -D {output_dir}
#SBATCH -o logs/sp_%A_%a.out
#SBATCH -e logs/sp_%A_%a.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --mem={mem}
#SBATCH -t {time_limit}
...
def _write_submit_script(
    ...
    lines = [
        ...
        "# Genere par setup_sp_calculations.py — ne pas editer a la main.",
        "set -e",
        "",
    ]
...
def setup(
    ...
    if not qbc_dir.is_dir():
        logger.error("--qbc-dir n'est pas un dossier : %s", qbc_dir)
        return 1
    ...
    poscar_files = _find_poscars(qbc_dir, output_dir, exclude, logger)
    calc_dirs = _prepare_calc_dirs(
        poscar_files, qbc_dir, output_dir, incar_sp, kpoints, potcar,
    ...
    slurm_path = _write_slurm_template(
        dirlist, output_dir, calcs_per_task, time_limit, mem, vasp_module, logger
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="setup_sp_calculations",
        description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("-i", "--qbc-dir",    default=str(QBC_POSCAR_DIR),
    ...
    parser.add_argument("--incar-sp",   default=str(INCAR_SP))
    ...
    parser.add_argument("--calcs-per-task", type=int, default=CALCS_PER_TASK,
                        help=f"Calculs SP par tache, executes en sequence "
    ...
    parser.add_argument("--unique-names", action="store_true",
                        help="Nommer les dossiers de calcul d'apres le chemin "
                             "relatif complet (evite les collisions entre runs QBC).")
    ...
    parser.add_argument("--mem",         default="2G",       help="Memoire SLURM.")
    ...
    qbc_dir    = _abs(args.qbc_dir)
    output_dir = _abs(args.output_dir)

    logger.info("QBC POSCARs : %s", qbc_dir)
    logger.info("Output      : %s", output_dir)

    return setup(
        qbc_dir        = qbc_dir,
        output_dir     = output_dir,
        incar_sp       = _abs(args.incar_sp),
