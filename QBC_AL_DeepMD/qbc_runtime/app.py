...
import os, glob, shutil, math, json, tempfile, time, sys
...
def read_input(path: str) -> Dict[str, Any]:
    print(f"[INFO] Reading input configuration from: {Path(path).resolve()}")
    cfg: Dict[str, Any] = {}
...
def committee_predict_batch(models, batch_atoms, type_map):
    """
    Évalue le comité sur un batch de frames en un seul appel par modèle.
    Retourne E (M, B), F (M, B, N, 3), V (M, B, 9).
    Beaucoup plus rapide que d'appeler committee_predict frame par frame.
    """
    all_coords = np.stack([
        a.get_positions().astype(np.float64).reshape(-1) for a in batch_atoms
    ])  # (B, N*3)
    all_cells = np.stack([
        a.cell.array.astype(np.float64).reshape(-1) for a in batch_atoms
    ])  # (B, 9)

    allowed = symbols2numbers(type_map)
    Z = batch_atoms[0].get_atomic_numbers()
    lut = {Zsym: i for i, Zsym in enumerate(allowed)}
    atype = np.array([lut[z] for z in Z], dtype=np.int32)

    B = len(batch_atoms)
    E_list, F_list, V_list = [], [], []
    for m in models:
        e, f, v = m.eval(all_coords, all_cells, atype)
        E_list.append(np.asarray(e).flatten())                          # (B,)
        F_list.append(np.asarray(f).reshape(B, -1, 3))                 # (B, N, 3)
        V_list.append(np.asarray(v).reshape(B, -1))                    # (B, 9)

    return (
        np.stack(E_list, axis=0),   # (M, B)
        np.stack(F_list, axis=0),   # (M, B, N, 3)
        np.stack(V_list, axis=0),   # (M, B, 9)
    )
...
#        seq = seq[SKIP::STRIDE]
        ...
        index_str = f"{SKIP}::{STRIDE}"
        t0 = time.time() #
        if ASE_FORMAT:
            read_kwargs = {"format": ASE_FORMAT, "index": index_str}
            if ASE_FORMAT not in _FORMATS_NO_SPECORDER:
                read_kwargs["specorder"] = TYPE_MAP
            seq = read(fpath, **read_kwargs)
        else:
            seq = read(fpath, index=index_str)
        if not isinstance(seq, (list, tuple)):
            seq = [seq]
        t_read = time.time() - t0 #
        print(f"[TIME] read={t_read:.3f}s  ({len(seq)} frames retenues)  {fpath}") #   
        ...
        for batch_start in range(0, len(seq), BATCH_SIZE):
            if LIMIT is not None and gidx >= LIMIT:
                break
            batch = seq[batch_start:batch_start + BATCH_SIZE]

            t1 = time.time() #
            E_b, F_b, V_b = committee_predict_batch(COMMITTEE, batch, TYPE_MAP)
            t_scoring_file += time.time() - t1 #

            for k, at in enumerate(batch):
                if LIMIT is not None and gidx >= LIMIT:
                    break
                ...
                dF, dF_max, dE, dV, std_f = disagreement_components(
                    E, F, V,
                ...
                atoms_bank.append(at)
                stdF_bank.append(std_f.astype(float))
    ...
    print(f"[TIME] scoring={t_scoring_file:.3f}s pour ce fichier") #
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: app.py <path/to/input.in>", file=sys.stderr)
        sys.exit(2)
    main(sys.argv[1])
